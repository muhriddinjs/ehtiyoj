...
export default function MasjidPage() {
  ...
  const TAG_OPTS = ["Hammasi", "Masjid", "5 mahal", "Juma", "Tahorat bor"];
  ...
  const loadPlaces = async (lat: number, lon: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/places?lat=${lat}&lon=${lon}&type=masjid&r=5000`);
  ...
  useEffect(() => {
    ...
    if (distFilter !== "Hammasi") {
      const maxM = distFilter === "500 m" ? 500 : distFilter === "1 km" ? 1000 : distFilter === "2 km" ? 2000 : 5000;
      result = result.filter((p) => p.distM <= maxM);
    }
    if (tagFilter !== "Hammasi") result = result.filter((p) => p.tags.includes(tagFilter));
    setFiltered(result);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <header style={{
        ...
        <div>
          <div style={{ fontSize: 19, fontWeight: 800 }}>Masjid</div>
          <div style={{ fontSize: 11, color: "var(--muted)" }}>Yaqin masjidlar</div>
        </div>
        <button onClick={findNearest} style={{
      ...
      <div style={{ padding: "0 0 88px" }}>
        {nearest && (
          <div className="page-enter" style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "var(--muted)" }}>{distLabel(nearest.distM)} • {nearest.tags.join(", ")}</div>
            </div>
          </div>
        )}

        <div className="search-wrapper" style={{ margin: "12px 16px 8px" }}>
          <span className="search-icon" style={{ fontSize: 16 }}>🔍</span>
          <input className="input-field" style={{ paddingLeft: 44 }}
            placeholder="Masjid qidirish..." value={search}
        ...
        <div style={{
          ...
          {(["map", "list"] as const).map((v) => (
            <button key={v} onClick={() => setView(v)} style={{
              flex: 1,
              background: view === v ? "linear-gradient(135deg, #059669, #34D399)" : "transparent",
        ...
        <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "0 16px 8px", scrollbarWidth: "none" }}>
          {DIST_OPTS.map((d) => (
            <button key={d} onClick={() => setDistFilter(d)} className={`chip ${distFilter === d ? "active" : ""}`}>{d}</button>
          ))}
        </div>

        <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "0 16px 10px", scrollbarWidth: "none" }}>
          {TAG_OPTS.map((t) => (
            <button key={t} onClick={() => setTagFilter(t)} className={`chip ${tagFilter === t ? "active" : ""}`}>{t}</button>
          ))}
        </div>
